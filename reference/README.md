...
`sim_v10.py` is the Python twin: the oracle of the current gate, lineage
**v0.10**. Under the twin-implementation regime, every rules increment lands in
...
The v0.10 delta — the Great Ridge length range rises to 5..10, so a Great
Ridge can no longer come out shorter than a lucky Ridge — is specified in
`docs/FORK_NOTES.md` §v0.10. It supersedes `sim_v09.py`.

`sample_log_seed42_v10.txt` is the twin's own **canon** output for `--seed 42
--eras 20` (LF line endings), the browser-smoke golden. Regenerate
only to prove the twin unchanged:

```bash
python reference/sim_v10.py --seed 42 --eras 20 --out /tmp/oracle --no-render
```

`history-v0.4/`, `history-v0.5/`, `history-v0.7/`, `history-v0.8/` and `history-v0.9/` hold the retired fixtures
