...
## v0.6: the experimental fields dial (promoted to canon in v0.8, below)
...
* v0.8 canon, 20 seeds at 20 eras: panels 34.9 (sd 1.1), reworks per panel 96.2 (sd 4.2), water 37.6 (sd 11.2, band 8.9 to 58.9). Panels and patina are unmoved from v0.7, so the map's growth and depth are untouched; water stays inside its own wide noise band, as the measurement rule requires us to say.
* What the promotion actually bought, the reason it was voted in: people per world now read fields low 3.8, fields high 20.7, rural 31.2, urban low 16.2, urban medium 8.2, urban high 4.3, and 18 of 20 worlds carry an urban high. Before the change a metropolis appeared in roughly one world in five. The farming landscape survived too: about 24 fields per world, most of them deep, because the growth step now deepens before it spreads.

## v0.8, verified in the mono-repo (the engine's own measurement)
* Same twenty seeds, twenty eras, run against the pre-promotion engine and the promoted one, so the deltas below are same-seed and not a seed-set artifact.
* Unmoved, as the package claims: panels per world 35.0 to 34.8; patina 95.8 to 96.5 embellishment steps per panel. Water 39.9 to 38.2, quoted only to say it moved less than its own noise; per the measurement rule it is not a finding.
* Moved, and this is what the promotion bought: fields low 39.0 to 3.5 and fields high 3.1 to 21.8 (the farmland survives and deepens, about 25 fields per world); rural 41.6 to 34.0; urban low 15.1 to 16.3; urban medium 4.7 to 9.3; urban high 0.6 to 5.0; worlds carrying an urban high 6/20 to 15/20.
* On the seed set used here the metropolis rate lands at 15/20 rather than the package's 18/20, and the pre-promotion rate at 6/20 rather than one in five. Same direction, same size; the difference is which twenty seeds, and the spread of urban high across worlds (sd 5.6, band 0-24) says the rate is a noisy statistic that should be quoted from its own band, like water.
* METRIC NOTE, on the record: "reworks per panel" in these notes is the EMBELLISHMENT steps per panel from the final report, not the report's own `reworks` counter (which reads about 71 per panel on the same runs). Two different numbers have been carried under one name since v0.5. Quote the one you measured.

## The error paths, second finding: a config no one can play
* The web-flavor ERROR-PATH suite (v0.8 rider) found the sibling of the -fexceptions bug. A config with an impossible geometry (a 0x0 panel) was accepted, built a world, and then hit an assertion deep inside the first age — abort in the browser, exit 3 on the CLI. Nothing between the caller and that assertion checked the value.
* Fixed where every surface passes through it: the Sim constructor validates the config and throws. The CLI prints "config: panel is 0x0; each side must be 2..64"; the FFI's existing catch turns the same throw into a null handle the app can report. No log byte of any valid config changes, and the gate stayed green through the fix.
* The rule this earns: a value a caller can supply may never reach an assert. Asserts are for the engine's own invariants.
